...
function sharpeye.HUDPaint()
	...
	if self:IsCrosshairEnabled() then
		...
		-- Calculating
		self.dat.crosshair.traceLineData = util.GetPlayerTrace( LocalPlayer(), vgui.IsHoveringWorld() and ((LocalPlayer().GetCursorAimVector != nil) and LocalPlayer():GetCursorAimVector() or nil) or LocalPlayer():GetAimVector() )
